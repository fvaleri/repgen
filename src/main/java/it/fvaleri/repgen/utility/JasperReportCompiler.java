...
public class JasperReportCompiler implements ReportCompiler {
    ...
    private byte[] compile(ReportType type, Map<String, Object> pars, Map<String, Object> reps, List<String> subs) {
        try {
            ...
            if (reps != null && reps.size() > 0) {
                for (String name : reps.keySet()) {
                    LOG.trace("Compiling report {}", name);
                    JasperDesign jdes = JRXmlLoader.load(templateHome + name + ".jrxml");
                    JasperReport jrep = JasperCompileManager.compileReport(jdes);
                    JRDataSource jrds = (JRDataSource) new JRBeanCollectionDataSource((List) reps.get(name), false);
    ...
    public static class Builder implements ReportCompiler.Builder {
        ...
        @Override
        public Builder withInMemory(boolean enable) {
            compilerTmp.inMemory = enable;
            return this;
        }
        ...
        public ReportCompiler build() {
            ...
            compiler.inMemory = compilerTmp.inMemory;
            return compiler;
        }
    }
}
