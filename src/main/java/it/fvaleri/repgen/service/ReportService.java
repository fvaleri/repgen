...
public class ReportService {
    ...
    public void setUp() {
        this.reportCompiler = new JasperReportCompiler.Builder()
            .withLocale(Locale.forLanguageTag(config.getLocale()))
            .withTemplateHome(config.getTemplateHome())
            .withInMemory(config.getInMemory())
            .build();
    ...
    public void buildReport(Report report) {
        ...
        try {
            ...
            if (modelName != null && activeModels.contains(modelName)) {
                ...
                report.setFilename(CommonHelper.sha256(content));
    ...
    public Search searchReports(Search search) {
        ...
        search.setReports(CommonHelper.sortReports(reports));
